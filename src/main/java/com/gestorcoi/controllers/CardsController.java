...
import com.gestorcoi.excel.DadosCardsAuxiliar;
import com.gestorcoi.excel.LeitorDadosCardInterrupcao;
...
public class CardsController {
	...
	private LeitorDadosCardInterrupcao leitorDadosCardInterrupcao = new LeitorDadosCardInterrupcao();
	...
	public void salvarAmarelo() throws Exception {
		...
		this.yellowCard.setDataInicio(redCard.getDataInicio());
		...
		if (this.yellowCard.getDataInicio() == null || this.yellowCard.getDataParcial() == null
			...
			this.yellowCard.setUnidadesInterrompidas(
					this.redCard.getUnidadeInterrompidas() - this.yellowCard.getUnidadesNormalizadas());
	...
	public void salvarVerde() throws Exception {
		...
		try {
			...
			this.greenCard.setDataInicio(redCard.getDataInicio());
	...
	public void carregarInterrupcaoDadosPlanilha() {

		try{
			DadosCardsAuxiliar dadosCardsAuxiliar = new DadosCardsAuxiliar();
			dadosCardsAuxiliar = leitorDadosCardInterrupcao.carregarDados(redCard.getCodigoEquipamento());

			redCard.setDistribuidora(dadosCardsAuxiliar.getDistribuidora());
			redCard.setRegional(dadosCardsAuxiliar.getRegional());
			redCard.setUnidadeInterrompidas(Integer.parseInt(dadosCardsAuxiliar.getQtd_uc()));
			redCard.setSuperintendencia(dadosCardsAuxiliar.getSuperintendencia());
			redCard.setSe(dadosCardsAuxiliar.getSubestacao());
			redCard.setAlimentador(dadosCardsAuxiliar.getAlimentador());
			redCard.setTensaoKva((((double) dadosCardsAuxiliar.getTensao()) / 1000));
			redCard.setLocaisAtingidos(dadosCardsAuxiliar.getLocais_atingidos());
		}catch(Exception e) {
			
			redCard.setDistribuidora("");
			redCard.setRegional("");
			redCard.setUnidadeInterrompidas(null);
			redCard.setSuperintendencia("");
			redCard.setSe("");
			redCard.setAlimentador("");
			redCard.setTensaoKva(null);
			redCard.setLocaisAtingidos("");
			MensagensJSF.msgSeverityInfo("Não foi possível encontrar esse equipamento na base de dados", "Não encontrado");
		}
	}
		...
		if (this.filterRedCardPorData != null && !this.filterRedCardPorData.toString().trim().equalsIgnoreCase("")) {
			...
			for (RedCard redCard : listaFiltradaEtapa1) {
				if (redCard.getDataInicio().getDay() == this.filterRedCardPorData.getDay()
						&& redCard.getDataInicio().getMonth() == this.filterRedCardPorData.getMonth()
						&& redCard.getDataInicio().getYear() == this.filterRedCardPorData.getYear()) {
